...
// the pushState monkey-patch). All overlay elements carry `data-relay-overlay`
...
// __relay_injectOverlay() is a no-op if the overlay already exists, and is
// called lazily by every action so a fresh page load (which clears injected
// DOM) re-installs it.

(function() {
    if (window.__relay_overlay_installed) return;

    var STYLE_ID = '__relay_overlay_style';
    var CURSOR_ID = '__relay_cursor';
    var RIPPLE_ID = '__relay_ripple';
    var HIGHLIGHT_ID = '__relay_highlight';
    var CARET_ID = '__relay_caret';
    ...
    function injectStyle() {
        ...
        style.id = STYLE_ID;
        style.setAttribute('data-relay-overlay', '');
        style.textContent = [
            '#' + CURSOR_ID + ' {',
            ...
            '}',
            '#' + CURSOR_ID + '.__relay_visible { opacity: 1; }',
            '#' + RIPPLE_ID + ' {',
            ...
            '}',
            '#' + HIGHLIGHT_ID + '.__relay_visible { opacity: 1; }',
            '#' + CARET_ID + ' {',
            '  position: fixed; pointer-events: none; z-index: 2147483647;',
            '  width: 2px; height: 18px; background: ' + ACCENT + ';',
            '  animation: __relay_blink 1s steps(2, start) infinite;',
            '  opacity: 0;',
            '}',
            '#' + CARET_ID + '.__relay_visible { opacity: 1; }',
            '@keyframes __relay_blink { to { visibility: hidden; } }'
        ].join('\n');
    ...
    function ensureEl(id, extraAttrs) {
        ...
        el.id = id;
        el.setAttribute('data-relay-overlay', '');
    ...
    window.__relay_injectOverlay = function() {
        injectStyle();
        ensureEl(CURSOR_ID);
        ensureEl(RIPPLE_ID);
        ensureEl(HIGHLIGHT_ID);
        ensureEl(CARET_ID);
        window.__relay_overlay_installed = true;
    };
    ...
    window.__relay_tweenCursor = function(toX, toY, durationMs) {
        return new Promise(function(resolve) {
            window.__relay_injectOverlay();
            ...
            cursor.classList.add('__relay_visible');
    ...
    window.__relay_showRipple = function(x, y) {
        window.__relay_injectOverlay();
    ...
    window.__relay_highlight = function(rect) {
        window.__relay_injectOverlay();
        ...
        hl.classList.add('__relay_visible');
    };

    window.__relay_fadeHighlight = function() {
        var hl = document.getElementById(HIGHLIGHT_ID);
        if (hl) hl.classList.remove('__relay_visible');
    };

    // Public: show/hide the synthetic caret at (x,y) during typing.
    window.__relay_showCaret = function(x, y) {
        window.__relay_injectOverlay();
        var caret = document.getElementById(CARET_ID);
        if (!caret) return;
        caret.style.left = x + 'px';
        caret.style.top = y + 'px';
        caret.classList.add('__relay_visible');
    };
    window.__relay_hideCaret = function() {
        var caret = document.getElementById(CARET_ID);
        if (caret) caret.classList.remove('__relay_visible');
    };
    ...
    var NARRATION_ID = '__relay_narration';
    var narrationTimer = null;
    window.__relay_narrate = function(text) {
        if (!text) return;
        window.__relay_injectOverlay();
        ...
        if (!el) {
            ...
            css.setAttribute('data-relay-overlay', '');
    ...
    window.__relay_overlay_installed = false;
