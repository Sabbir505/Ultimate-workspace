...
A local-first desktop shell for AI coding agents with ONE unified chat surface (the old separate Dev/Chat tabs were removed in the single-mode layout rework, `d39d5a25`). It does **not** implement its own agent loop for harness CLIs — it orchestrates existing CLI binaries, and adds a direct-HTTP LLM chat backend for the built-in/local agents. The product is "Relay" everywhere the user sees it; the Rust crate, the bundle id, the installer, the mobile app, and the scheduled task are still "Relay" (see the naming note at the top of this file).
...
| Concern | Files |
|---|---|
| Backend entry | `src-tauri/src/lib.rs`, `src-tauri/src/main.rs`, `src-tauri/src/bin/relay_automation.rs` (headless runner binary) |
