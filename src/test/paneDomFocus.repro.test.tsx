...
describe("pane focus moves DOM focus to the terminal", () => {
  ...
  it("focusPane moves document.activeElement to the target terminal's textarea", () => {
    ...
    // Let xterm mount + its focus effect run for the initially-focused pane.
    const textareas = () => container.querySelectorAll("textarea.xterm-helper-textarea");
    // Wait a tick for xterm to open + focus effect to fire.
  ...
  it("pressing Mod+1 (Ctrl+1) moves DOM focus to pane 0's terminal even when xterm stopPropagation's the keydown", () => {
  ...
  it("Mod+1 re-runs the focus effect (focusEpoch bump) even when pane 0 is already focused", () => {
    useSettingsStore.setState({ theme: "dark", keybindings: { ...DEFAULT_KEYBINDINGS } });
    const a = usePanesStore.getState().addPane(terminalDesc("s1"));
