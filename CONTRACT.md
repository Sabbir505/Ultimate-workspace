...
- `ChatConfigPayload { provider: string | null, baseUrl: string | null, model: string | null }`
...
- `send_chat_message(chatSessionId: string, content: string) -> ()` — persists user message, looks up provider/model/api_key, assembles message history, kicks off SSE streaming. Emits `chat:token`, then `chat:done` or `chat:error`.
