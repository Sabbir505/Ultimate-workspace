...
| P2 | ~~**E2E encryption on the relay**~~ **DONE** — HKDF-SHA256 + XChaCha20-Poly1305 session keyed from the pairing token (proof-only pairing: raw token never on the wire; phone sends HMAC proof, both sides derive the key). Rust: `relay_crypto` + crypto embedded in the shared sink (relay_ws) so request loop, owner-channel pump, and inbound decrypt share one atomic counter state. Mobile: `relayCrypto.ts` (@noble/ciphers + @noble/hashes) with auto-fallback to legacy raw-token pairing for pre-E2E desktops. Cross-implementation byte-exact vectors pinned in `noble_cross_implementation_vectors`; legacy plaintext mode kept for old phones; §3.2.11 | §3.2.11 — match Happy Coder | M |
| P2 | ~~**MCP server gallery for built-in chat**~~ **DONE** — new `mcp_gallery` module: 8-entry curated catalog (filesystem/memory/sequential-thinking/everything/fetch/git/sqlite/time) + custom stdio servers, defs persisted in `mcp.servers` KV, live rmcp `TokioChildProcess` sessions cached across turns (Windows cmd.exe /C shim wrapping; kill-all on app exit; self-healing reconnect bounded at 30s). Tools attach to every tool-enabled chat turn under collision-safe `mcp_<server>_<tool>` wire names, classified Read/Write with the SAME approval gating as connectors (Writes stripped under read_only, approval cards under manual). Settings → MCP Servers panel (gallery cards, enable toggles, Connect w/ tool preview incl. read/write chips, custom-server form). Verified live: ignored cargo test spawns server-everything via npx and round-trips tools/list + tools/call echo; §3.2.14 | §3.2.14 | M |
| P3 | **macOS build job** — *compile-only CI check shipped (`.github/workflows/build.yml` `check-macos`); a release bundle (.dmg) remains open* (when growth > focus) | §3.2.12 | M–L |
