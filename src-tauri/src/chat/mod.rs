...
pub(crate) async fn run_chat_stream(
    ...
    // no perf recording here — the SSE pump path records separately. The
    // app-exit partial buffer (partial_buf) mirrors the stream so a quit
    // mid-stream can persist it.
