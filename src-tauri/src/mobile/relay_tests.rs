...
async fn e2e_ws_pair(
    ...
    let write: crate::mobile::relay_ws::SharedWsWrite =
        Arc::new(tokio::sync::Mutex::new(crate::mobile::relay_ws::SinkState {
            sink,
...
fn phone_frame(key: &[u8; 32], counter: u64, msg: &crate::mobile::protocol::MobileMessage) -> Vec<u8> {
...
async fn mid_turn_encrypted_cancel_is_honored_and_counter_stays_in_sync() {
    ...
    let cancel = MobileMessage::CancelChatTurn { chat_session_id: "cs-1".into() };
    handle_mid_turn_frame(Message::Binary(phone_frame(&key, 0, &cancel)), true, &record, &write).await;
    assert_eq!(*cancelled.lock(), vec!["cs-1".to_string()], "cancel must reach ChatManager");
    match phone_recv(&mut client, &key, 0).await {
        DesktopMessage::ChatDone { chat_session_id, usage } => {
    ...
    handle_mid_turn_frame(Message::Binary(phone_frame(&key, 1, &other)), true, &record, &write).await;
    ...
    assert_eq!(cancelled.lock().len(), 1, "non-cancel commands must not cancel");
    ...
    match phone_recv(&mut client, &key, 2).await {
        DesktopMessage::ChatError { error, .. } => assert!(error.contains("undecryptable"), "{error}"),
        other => panic!("expected undecryptable ChatError, got {other:?}"),
    }
    assert_eq!(cancelled.lock().len(), 1, "a replayed frame must not cancel");

    // ...phone frame #3 at counter 3 still decrypts and cancels.
    let cancel2 = MobileMessage::CancelChatTurn { chat_session_id: "cs-2".into() };
    handle_mid_turn_frame(Message::Binary(phone_frame(&key, 3, &cancel2)), true, &record, &write).await;
    assert_eq!(*cancelled.lock(), vec!["cs-1".to_string(), "cs-2".to_string()]);
    match phone_recv(&mut client, &key, 3).await {
        DesktopMessage::ChatDone { chat_session_id, .. } => assert_eq!(chat_session_id, "cs-2"),
        other => panic!("expected ChatDone, got {other:?}"),
    }
...
async fn mid_turn_plaintext_frame_on_e2e_connection_is_rejected() {
    ...
    assert!(cancelled.lock().is_empty(), "plaintext on E2E must not be honored");
    match phone_recv(&mut client, &key, 0).await {
...
fn session_chat_event_listener_slot_is_claimed_exactly_once() {
    ...
    assert!(claim_listener_slot(&flag), "first relay start registers the listener");
    assert!(!claim_listener_slot(&flag), "second relay start must NOT register another");
...
fn transcript_hash_is_stable_across_polls() {
    ...
    assert!(unchanged_second, "second poll of a static screen must report unchanged");
...
fn temp_chat_session_cleanup_deletes_session_and_messages() {
    ...
    db::add_chat_message(&conn, &cs.id, "user", "hi", None, None, None, None, None, None, None, None, None, None, None, None, None, None, None).unwrap();
