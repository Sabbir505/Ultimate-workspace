...
- `set_chat_session_auto(chatSessionId: string, auto: boolean) -> ()` — flips a chat between **Auto model routing** and a pinned provider/model (AUTO_MODEL_ROUTING_RESEARCH.md). `true` marks the session auto-routed and resets provider/model to `"auto"` placeholders; `false` clears the flag so a manual pick takes over. On every send into an auto-routed session the backend resolves provider+model via `chat/auto_router.rs` (cloud providers only — local GGUF and harness CLIs are never auto candidates), writes the resolution back to the row (display, context meter, cost attribution, next-turn stickiness), emits `chat:status` with `reason: "auto_route"` disclosing the pick + reason, and carries the remaining chain as fail-over candidates: a PRE-stream failure with a retryable class (401/402/429/5xx/network/dead model) is recorded in the health store (`chat/model_health.rs`, `chat.auto.health` setting) and the next candidate takes the turn with a disclosed `chat:status`; MID-stream failures never re-route. Bias setting `chat.auto.bias` = `"quality" | "balanced" | "economy"` (default balanced) drives cost-aware ranking (the picker's Auto pane footer writes it).
