...
// raw-byte streams. Why: `emit` is a global broadcast (every listener in
// every window gets the message) + each event is JSON-serialized on the
// Rust side. For PTY output (50-200 events/sec, each up to 64 KB) the JSON
// cost is the dominant per-keystroke CPU. `Channel<T>` is a typed,
// point-to-point stream — one sender, one receiver, no global fan-out, no
// JSON envelope.
//
// Frontend usage:
...
//
// In tests / when the Tauri runtime is absent, `ptyChannel` rejects and
// the caller falls back to `safeListen("pty:output", ...)` (see
// `TerminalPane.tsx`).
