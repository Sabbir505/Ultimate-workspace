...
- **MessageBubble.tsx** — ✅ done (session 10): carved to 708 lines; the Markdown rendering system (lazy highlighter plumbing, icons, ChatImage, citeUrlTransform, <Markdown> + InsidePreContext) and the whole activity-step stack (StepStatusIcon/stepLabel/InlineDiff/StepCodeHighlighter/ActivityStepRow/ProcessSummary/FoldedStepGroup/EditFileRow/ThinkingBlock + ActivityStep/Group/Block types, groupSegments, renderProcessBlock, useLiveElapsedSec, formatMessageTimestamp) now live in ActivitySteps.tsx (1,676 lines), imported back one-directionally; ThinkingBlock is re-exported for SubagentPanel.

- **ChatComposer.tsx** — ✅ voice engine (session 8, `lib/voiceRecording.ts`) + helpers/attachment plumbing (session 10, `composerShared.tsx`, 3,032→2,746). Remainder: the ~2,700-line component body.
