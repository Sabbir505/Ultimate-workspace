...
The core project docs live in the `AI CONTEXT/` folder: see `PRD.md` for the full product spec, `CONTRACT.md` for the frontend/backend IPC contract, `AI_CONTEXT.md` for the canonical AI-facing code map, `BUILD_LOG.md` for build history, and `RELEASE.md` for the release/auto-update workflow. Historical audits and design records also live here (`AUDIT.md`, `BUG_LIST.md`, `BUG_LIST_ROUND2.md`, `COST_MODEL_REDESIGN.md`); the root-level `BUG_AUDIT.md`, audit/research records, and architecture docs (`DOCUMENT_DESIGN_ARCHITECTURE.md`, `MEMORY_DESIGN_ARCHITECTURE.md`, …) live at the repo root.
...
- **Persistence:** SQLite (projects, sessions, cost events, skills, quick actions, settings, memory, self-improvement) — 42 tables, WAL mode
...
```bash
npm test                      # frontend logic tests (vitest, 100 files / 733 tests as of 2026-09-05)
cd src-tauri && cargo test --lib  # backend unit tests (865 passed, 0 failed, 12 ignored as of 2026-09-05)
```
