...
> **Note (2026-08-14):** The original proposal enumerated six browser ops and two `read_page` modes (`interactive | content`). The shipped implementation expanded the surface: the binary now advertises **10 browser ops** (`navigate` / `read_page` / `click` / `type_text` / `scroll` / `wait_for` / `screenshot` / `history` (back|forward) / `hover` / `evaluate` / `click_and_wait`) plus **5 conduit tools** (`generate_document` / `generate_diagram` / `generate_file` / `get_skill` / `list_skills`). The `type` tool was renamed `type_text`; `read_page` supports **four** `ReadMode` variants (`full` / `summary_only` / `section` / `interactive`). The original task spec below remains accurate for the *intent* and *architecture* (loopback WS, single-pane eval bridge, per-project `.mcp.json` registration); only the tool count and naming have grown. See the current dispatch in `src-tauri/src/browser_mcp.rs` and the MCP server in `src-tauri/src/bin/conduit_browser_mcp.rs` for the shipped surface.
