...
|---|---|---|---|
| 1 | ~~**Per-session permission modes are unwired**~~ **RESOLVED** — the permission rewire (`ff0b812f`, 2026-08-15) wired the full chain: `chat_sessions.permission_mode` column (default `manual`, fail-closed `from_db` load at send), `update_chat_session_permission_mode` IPC, `PermissionModeMenu` in the composer footer, `ApprovalCard`/`FullAutoConfirmModal`, approval rules engine (`permissions.rules` KV), and the Claude Code `can_use_tool` stdio relay. Kimi/OpenCode/ACP headless runs intentionally stay full-auto (no approval channel). | `src-tauri/src/chat/commands.rs` · `permission.rs` · `src/components/chat/` | ✅ Fixed (`ff0b812f`); `BUG_LIST` H2 closed. |
...
1. ~~**Wire per-session permission modes end-to-end (or remove the half-built UI).**~~ **DONE** (`ff0b812f`) — column + IPC + menu + approval cards + rules engine + Claude Code relay all shipped; Kimi/OpenCode/ACP intentionally full-auto.
