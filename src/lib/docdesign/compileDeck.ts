...
// content: a slide title like "Using #abcdef in CSS" lands in the program
// as a quoted string literal and would false-trip l2/hex-hash (audit #26).
// Blank string/template-literal contents before the check, but keep
// color-slot literals (`"1A2B3C"`, or a real `"#1A2B3C"` bug) intact so the
// invariant still catches actual '#'-prefixed hex colors in the skeleton.
...
export function checkInvariants(code: string, plan: DeckPlan): CompileChecks {
  ...
  //    Judged on the skeleton (model text is blanked — see skeletonOf).
