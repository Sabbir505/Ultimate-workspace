...
/** Sanitizer for the office converters' OWN output (docx fallback / xlsx /
 *  pptx previews — generated by our backend, not the model). DOMPurify's
 *  default mode returns only `<body>` content, which silently dropped the
 *  converters' `<head><style>` block and left every office preview
 *  unstyled. WHOLE_DOCUMENT keeps the head (and its stylesheet) while the
 *  script/attribute policy above still applies. */
export function sanitizeOfficeDocumentHtml(html: string | null | undefined): string {
  if (!html) return "";
  return DOMPurify.sanitize(html, { ...PURIFY_CONFIG, WHOLE_DOCUMENT: true });
}
