...
### Added
- **Native build updaters for the whisper/TTS CUDA builds (the harness-updater shape, applied to binaries)** — the three pinned native artifacts this app downloads and executes (whisper.cpp server CPU build, whisper.cpp CUDA build, sherpa-onnx CUDA TTS runtime) now have the same check→Update flow harness CLIs have: each installer stamps a `build-version.json` marker into its install dir, `check_build_updates` compares the marker against the version the app pins (the same SHA-verified constants the installers pull), and rows surface as Update buttons — Settings → Local Models → Speech (SttPanel/TtsPanel) — plus a once-per-open bell check (`checkAndNotifyBuildUpdates`, deduped per version in localStorage, deep-linking to the Speech panel) mirroring `checkAndNotifyHarnessUpdates`. Includes the previously missing one-click CUDA whisper build (`stt_install_cuda`, id `stt-whisper-cuda`): the pinned `whisper-cublas-12.4.0` asset from the SAME release tag the CPU zip uses, SHA-256 verified at pin time against the real asset, extracted flat into `bin/whisper-cpp-cuda` where `cuda_binary()` already resolves it — it bundles its own CUDA 12 runtime DLLs, so no CUDA toolkit install is needed (the GPU toggle's old "put a build in the folder yourself" guidance is gone). Existing installers gained a `force` flag so an update re-downloads over a live install (a running sidecar is stopped first — Windows won't overwrite a running image); the TTS GPU installer re-downloads both halves under force. Update buttons show optimistic "current" flips (`markBuildUpdated`) exactly like the harness rows.
